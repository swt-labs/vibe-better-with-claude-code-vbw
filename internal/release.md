...
- Find last release commit: `git log --extended-regexp --grep="^chore(\(release\))?: (release )?v[0-9]" --format="%H %s" | head -20`, then verify each candidate's **subject line** matches the pattern (since `git log --grep` searches the full commit message including body). Take the first subject-verified match as the hash (fallback: root commit). The `--extended-regexp` pattern matches `chore: release v{x}` (no scope), `chore(release): v{x}` (scoped), and `chore(release): release v{x}` (scoped with redundant prefix). The `(\(release\))?` group restricts the scope to exactly `(release)` or no scope, preventing false positives from other scopes like `chore(deps): v2.0.0`. The `(release )?v[0-9]` anchor excludes `chore(release): bump version` commits. **Limitation:** `git log --grep` matches any line in the commit message (subject + body), so the subject-verification step is required to avoid false positives from body lines like `chore: release v1.30.0 was the previous release`. Capture the match's date via `git log -1 --format=%Y-%m-%d {hash}` (fallback: empty string, which omits the `merged:>=` filter).
