...
- **Dry-run gate:** If `--dry-run` and entries were generated, show all generated entries below but do NOT write any files. Display "○ Dry run — no changes written." after showing suggestions. Skip insertion. If `--dry-run` and no entries were generated (N=0), display "✓ No undocumented changes — CHANGELOG.md is up to date." instead. Skip insertion.
...
- **Insertion (non-dry-run only) — write immediately, no user confirmation:** No remediation sub-step in this bullet gates on user input, except the README step which requires explicit confirmation. Perform the following in order:
  1. **Stale section cleanup (always runs, even when N=0):** Run `git fetch --tags --quiet`. If `git fetch --tags` exits non-zero (network failure, auth expired), display "⚠ Could not fetch tags from remote — skipping stale section cleanup to avoid deleting valid sections." and skip to sub-step 2. If fetch succeeds, scan CHANGELOG for `## [{version}]` headers where `{version}` matches a semver pattern (`[0-9]+\.[0-9]+\.[0-9]+`), has no corresponding git tag (the output of `git tag -l "v{version}"` is empty — note: `git tag -l` always exits 0 regardless of matches, so check stdout content, not exit code), and `{version}` differs from `{new-version}`. Skip any header that doesn't match the semver pattern (treat as non-version content, e.g., `## [Overview]`). Remove each stale section (header through content until next `## [` or EOF). Display: "ℹ Removed stale changelog section for v{version} (untagged)."
  2. **Short-circuit check:** If no entries were generated (N=0 from both PR-centric and commit fallback), skip insertion entirely. Display: "✓ No undocumented changes — CHANGELOG.md is up to date." Proceed directly to the README step.
  3. **Write entries:** Display the generated entries, then **immediately write them to CHANGELOG.md** in the same step. Do NOT wait for user confirmation, do NOT defer insertion, and do NOT combine this with the README prompt. Create a `## [{new-version}] - {release-date}` section header and insert it with the generated entries directly above the first existing `## [x.y.z]` entry. If no version entries exist, insert after the last non-empty line following the `# Changelog` header. If a `## [{new-version}]` section already exists (from a previous aborted run), append only entries whose PR number (or commit hash for fallback entries) does not already appear in the existing section. Do not remove or reorder existing entries in that section. Changelog insertion must be complete (file written to disk) before proceeding to the README step. After writing, display: "✓ CHANGELOG.md updated with {N} entries under ## [{new-version}]."
- **README (separate step, after changelog is written):** Show specific corrections, apply on confirmation only. README corrections require explicit user confirmation via AskUserQuestion. This is the only remediation sub-step that gates on user input.
...
Delete remote release branch (if not already deleted by `gh pr merge --delete-branch` in Guard 4): First check if the remote branch still exists: `git ls-remote --heads origin refs/heads/release/v{version}`. Check the exit code first: if `git ls-remote` exits non-zero (network error, auth failure, remote unreachable), display "⚠ Could not verify remote branch status — attempting deletion anyway." and fall through to the deletion attempt below (do not treat empty stdout from a failed command as "branch already gone"). If `git ls-remote` exits 0 and output is empty, the branch is already gone (likely deleted by the PR merge step); display "✓ Remote branch already cleaned up." and skip deletion. If `git ls-remote` exits 0 and output is non-empty, delete it: `git push origin --delete release/v{version} 2>&1`. If deletion succeeds, done. If deletion fails and stderr contains `remote ref does not exist` (TOCTOU race — branch was deleted between the ls-remote check and the push), treat as success: display "✓ Remote branch already cleaned up." If deletion fails with any other stderr, display: "⚠ Could not delete remote branch `release/v{version}` — delete manually." This is non-fatal (release is already tagged); continue to Step 5 regardless. **Note:** Guard 7 uses a different deletion strategy (stderr classification without pre-check) because it iterates over multiple branches from a pre-collected `git ls-remote` list and must handle partial cleanup. Step 4 handles a single known branch where the common case is "already deleted by PR merge," making a pre-check the cleaner approach.
