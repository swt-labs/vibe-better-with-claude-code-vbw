---
name: running-claude-smoke-tests
description: 'This skill should be used when the user asks to run or verify a VBW smoke test with plain Claude Code (`claude`), capture visible `Skills:` output, validate `.vbw-planning/.skill-decisions.log`, or confirm consumer-smoke-worktree versus `--plugin-dir` provenance for local smoke coverage in this repo.'
argument-hint: '[smoke-goal]'
---

# Running Claude Code smoke tests

Use this skill when validating VBW behavior with plain Claude Code so future issue-fix runs do not have to rediscover the local CLI entrypoint, the consumer repo path, or the provenance checks. This is the plain-Claude counterpart to the CCR smoke-test workflow: keep the worktree and provenance rules the same, but replace `ccr code` with `claude`.
...
- Create or reuse a **disposable consumer smoke worktree** and run `claude` there, or use a **sandbox repo** when the real consumer repo cannot exercise the flow.
...
- If the transcript path encodes the plugin repo or the base consumer repo checkout instead of the consumer smoke worktree or sandbox, treat the smoke result as suspect until proven otherwise.
...
- **Consumer repo** — the real target repo used to resolve the testing target and create or remove the smoke worktree. Do not run `claude` from this checkout.
...
- [ ] Run smoke command(s) with `claude`
...
- you need `/vbw:vibe --execute` or `/vbw:vibe --verify` against controlled phase or UAT state

When you split coverage this way, say so explicitly in the verification summary.

### 5. Use `claude` for this variant

Use the plain Claude Code entrypoint for this variant:

```bash
claude --help
```

We have not runtime-validated this path here. Treat `claude` as the expected local equivalent of `ccr code`, and confirm any flag assumptions with `claude --help` before relying on them.

### 6. Run `claude` from the consumer smoke worktree or sandbox
...
- Commit the **minimal seed fixtures** that define the bug premise before invoking `claude`.
...
If `claude -p "...prompt..."` reports an input error, switch to stdin and keep it that way for the run:

```bash
cd /absolute/path/to/consumer-smoke-worktree && \
printf '%s\n' '/vbw:fix Investigate a SwiftData save failure during tests.' \
  | claude -p --dangerously-skip-permissions --plugin-dir /absolute/path/to/candidate-plugin-checkout
```
...
Always capture:
- command exit code
- transcript or project path proving the smoke cwd was the consumer smoke worktree or sandbox

Then capture the claim-matched evidence you actually need:
- **Visible-selection claim** → capture a visible `Skills:` line
- **Activation or logging claim** → capture `.vbw-planning/.skill-decisions.log` deltas
- **Domain-skill claim** → capture task-specific keyword matches when relevant, such as `SwiftData` or `swiftdata`
...
Use this when you only need mode or skill-selection behavior and do not want the workflow to keep going:

```bash
cd /absolute/path/to/sandbox-repo && \
printf '%s\n' '/vbw:vibe --execute 1' \
  | claude -p --permission-mode plan --dangerously-skip-permissions \
      --plugin-dir /absolute/path/to/candidate-plugin-checkout \
...
Use these checks:
- The shell cwd for `claude` must be the consumer smoke worktree or sandbox.
- The transcript path should encode the consumer smoke worktree or sandbox path under `<claude-config-dir>/projects/...`, not the candidate plugin checkout or base consumer repo checkout.
  - Resolve `<claude-config-dir>` the same way this repo does: `CLAUDE_CONFIG_DIR` if set, else `$HOME/.config/claude-code` when that directory exists, else `$HOME/.claude`.
  - Consumer worktree example: `<claude-config-dir>/projects/-path-to-consumer-repo-worktrees-smoke-fix-issue-502/...`
  - Sandbox example: `<claude-config-dir>/projects/-private-tmp-vbw-skill-smoke-502/...`
...
- **Need real consumer coverage** → create or reuse the consumer smoke worktree before running any `claude` command.
...
- **Need visible `Skills:` output** → use stream-json plus `--verbose` and search the capture.
