...
EOF
  ...
  # Verify all defaults.json keys are present (38 defaults keys)
...
EOF
  ...
  # Verify flag count is correct (38 total, graduated flags removed)
  run jq 'keys | length' "$TEST_TEMP_DIR/.vbw-planning/config.json"
  [ "$status" -eq 0 ]
  [ "$output" = "38" ]
}
...
@test "EXPECTED_FLAG_COUNT is 38 after partial flag restoration" {
  # Verify session-start.sh has EXPECTED_FLAG_COUNT=38 (4 statusline flags + require_phase_discussion + auto_uat added)
  SCRIPT_COUNT=$(grep 'EXPECTED_FLAG_COUNT=' "$SCRIPTS_DIR/session-start.sh" | grep -oE '[0-9]+' | head -1)
  [ "$SCRIPT_COUNT" = "38" ]
}
