...
- **Forbidden:** Do not run commands that mutate files, git state, packages, services, databases, credentials, or external systems. Do not use Bash heredocs, redirection, `tee`, `sed -i`, or similar shell-based file writes. Do not use `eval`, command substitution (`$(...)` or backticks), or nested shell execution such as `bash -c` / `sh -c` because these forms can hide mutating payloads from Scout's read-only validation; call verified read-only helper scripts or curl wrappers directly instead. Never use Bash to create or edit research artifacts; use Write for the provided output path.
