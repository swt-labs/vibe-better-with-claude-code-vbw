...
3. **Process-exception** — the exception is documented with explicit non-fixable justification (verify documentation exists)

A remediation round that only adds justification text to SUMMARY.md `deviations:` arrays without addressing the underlying code/plan mismatch does NOT resolve the FAIL. If the VERIFICATION HISTORY lists FAIL checks that remain unaddressed by any of the three resolution paths, they are still FAIL checks in your verification.
