...
**If RESEARCH.md exists** (referenced in your prompt or found as `### Research Findings` in compiled context): Trust the research — the Scout already analyzed the codebase. Do NOT do broad exploratory scanning (no Glob sweeps, no Grep pattern searches, no LSP "find all references" trawls, no Read of files not named in the research). Only use targeted tool calls to verify specific claims if needed (e.g., confirming a line number is still current, checking a single definition). Proceed directly to Stage 2.
