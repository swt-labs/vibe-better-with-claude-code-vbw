...
0b. **Memory check (MANDATORY):** Call `muninn_guide(vault from .vbw-planning/config.json muninndb_vault)` on first use to get vault-aware instructions. Then call `muninn_activate(vault, context: "{bug description} {error message}", limit: 5)` to check for similar past issues. Review results before hypothesizing — a similar bug may have been resolved before.
