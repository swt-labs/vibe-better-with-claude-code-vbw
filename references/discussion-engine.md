...
Both are available to agents during execution. Milestone context is injected into agent context by `compile-context.sh`; phase context is read directly by agents from the phase directory. Milestone context gives agents the "why" behind the phase structure; phase context gives them the "what" for their specific phase.
