  ...
  **Summary-deviation checkpoint prompt:** If the current checkpoint is a prefilled `D{NN}` summary-deviation review, show the deviation text and source metadata instead of a product scenario.
  ...
  - **"Skip" selected:** record skip. For a prefilled summary-deviation `D{NN}` checkpoint, also write `**Disposition:** skipped-by-user` and do not record acceptance.
   - **Freeform text (via "Other"):** Apply case-insensitive, trimmed string matching:
     - For a prefilled summary-deviation `D{NN}` checkpoint, high-confidence todo intent (`/vbw:todo`, `todo`, `to-do`, `add to todo`, `add to to-do`, `track this`, `track it`, `backlog`, or `follow up later`) maps to the accepted-and-tracked path when the response does not include explicit rejection/blocking markers (`unacceptable`, `reject`, `blocking`, `blocker`, `do not continue`, `cannot continue`, `not okay`). If a rejection/blocking marker is present, continue to the issue path.
