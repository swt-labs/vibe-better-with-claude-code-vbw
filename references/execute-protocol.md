...
The plan_path argument is passed for context. compile-context.sh calls `evaluate-skills.sh` to read installed skill names from STATE.md's `**Installed:**` line, locate each skill's SKILL.md on disk (3 search paths: `$CLAUDE_DIR/skills/`, `.claude/skills/`, `~/.agents/skills/`), and extract the `description` field from YAML frontmatter. The result is a markdown table (`| Skill | Description |`) emitted into the compiled context for all roles (dev, lead, qa, scout, debugger, architect). Each agent reviews descriptions to decide which skills are relevant, then calls `Skill(skill-name)` only for matching skills — leveraging Claude Code's native progressive disclosure for on-demand loading. This description-based forced evaluation prevents false positives from name-only matching.
