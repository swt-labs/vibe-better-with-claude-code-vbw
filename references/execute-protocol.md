...
- **Ad-hoc paths (`/vbw:fix`, `/vbw:debug`, `/vbw:research`):** Debugger/Dev/Scout reads STATE.md's `**Installed:**` line directly — no plan exists, so no `skills_used` frontmatter to reference. Agent evaluates relevance and activates applicable skills.
