...
The plan_path argument is passed for context. Skill activation is handled separately by the `skill-evaluation-gate.sh` SubagentStart hook, which fires at agent spawn-time and injects a MANDATORY SKILL EVALUATION SEQUENCE into `additionalContext` (high attention position). The hook calls `evaluate-skills.sh` to read installed skill names from STATE.md's `**Installed:**` line, locate each skill's SKILL.md on disk (3 search paths: `$CLAUDE_DIR/skills/`, `.claude/skills/`, `~/.agents/skills/`), and extract the `description` field from YAML frontmatter. The result is a skills table injected at spawn-time with a 3-step gate (EVALUATE YES/NO per skill, ACTIVATE relevant ones via `Skill(skill-name)`, then IMPLEMENT). This hook-based approach fires for all agent spawns (including `/vbw:fix`) and achieves ~95% activation reliability by placing the instruction in a high-attention position rather than compiled context background text.
