...
require_grep "debug-start helper has legacy flat session fallback" 'debugging"' "$DEBUG_START_SCRIPT"
require_grep "debug-start helper exposes pickup failure after session creation" 'pickup_failed_after_session' "$DEBUG_START_SCRIPT"
...
require_grep "debug documents status-variant helper schema" 'Helper output schema is status-variant' "$DEBUG_CMD"
require_grep "debug parses status before branch-specific helper fields" 'Always parse `\.status` before reading branch-specific fields' "$DEBUG_CMD"
require_grep "debug documents full selected payload for success statuses" 'Success-like payloads \(`ok` and `already_complete`\) include the full selected-todo payload' "$DEBUG_CMD"
require_grep "debug documents minimal no-session error payloads" 'No-session errors may carry only `status`, `code`, and `message`' "$DEBUG_CMD"
require_grep "debug documents session-bearing error payloads" 'Session-bearing errors carry `status`, `code`, `message`, `mode`, `todo_selected`, `session`, and usually `pickup`' "$DEBUG_CMD"
