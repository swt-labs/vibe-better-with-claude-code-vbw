...
if grep -qi 'eval' "$SCOUT_FILE" && grep -qi 'command.*substitution' "$SCOUT_FILE" && grep -qi 'process.*substitution' "$SCOUT_FILE" && grep -qi 'nested shell execution' "$SCOUT_FILE" && grep -qi 'control syntax\|grouping' "$SCOUT_FILE"; then
