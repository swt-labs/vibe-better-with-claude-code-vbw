...
4. **Spawn investigation:**
    **Path A: Competing Hypotheses** (prefer_teams='always' OR (effort=high AND ambiguous)):
    ...
    - Spawn 3 vbw-debugger teammates, one task each. **Add `model: "${DEBUGGER_MODEL}"` to each Task spawn. If `DEBUGGER_MAX_TURNS` is non-empty, also pass `maxTurns: ${DEBUGGER_MAX_TURNS}`. If `DEBUGGER_MAX_TURNS` is empty, do NOT include maxTurns (omitting it = unlimited).**
    ...
    **Path B: Standard** (all other cases):
    - Resolve Debugger model:
        ...
        ```
    - Display: `◆ Spawning Debugger (${DEBUGGER_MODEL})...`
    - Spawn vbw-debugger as subagent via Task tool. **Add `model: "${DEBUGGER_MODEL}"` to the Task tool invocation. If `DEBUGGER_MAX_TURNS` is non-empty, also pass `maxTurns: ${DEBUGGER_MAX_TURNS}`. If `DEBUGGER_MAX_TURNS` is empty, do NOT include maxTurns (omitting it = unlimited).**
        ```text
