...
3. **Research persistence (REQ-08, graduated):** If effort != turbo:
   - Determine the next plan number `{MM}` and resolve artifact paths:
     ...
     ```
   - Check for phase-wide research `{phase-dir}/${RESEARCH_NAME}` (preferred) or per-plan `{phase-dir}/{NN}-{MM}-RESEARCH.md` (brownfield fallback).
