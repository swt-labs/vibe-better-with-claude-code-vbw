...
5. **Resolve remediation stage (single call):**
   ...
   **Run directly (do NOT capture with `$()`).**
   - Both resume and init paths emit **round metadata** after the stage line:
     ```text
     round=RR              — zero-padded current remediation round (e.g., 01)
     round_dir=<path>      — absolute path to current round directory
     ...
     ```
     **Use these values directly** — do NOT glob `*-PLAN.md` or search for RESEARCH.md files. The script pre-computes all four from the phase directory. Remediation artifacts are named `P{NN}-R{RR}-RESEARCH.md` and `P{NN}-R{RR}-PLAN.md` inside the round dir (where NN = phase number, RR = round number).
   - If a stage was already persisted (resume after compaction/restart), the script returns the stage word + round metadata with no side effects.
   - If no stage existed (first entry into remediation), the script initializes the stage file, creates the round directory, pre-seeds CONTEXT.md, and returns the stage word + round metadata + `---CONTEXT---` separator with the full pre-seeded CONTEXT.md content — **use this directly as your remediation context. Do NOT separately read UAT.md or CONTEXT.md files.**
...
6. **Execute the current stage** based on `STAGE`:
   ...
   **Round metadata prohibition:** Do NOT glob `*-PLAN.md` or search for `*-RESEARCH.md` — use the pre-computed `round`, `round_dir`, `research_path`, and `plan_path` values from step 5.
   - `research`: If `research_path` from step 5 is non-empty, research already exists — skip to advancing the stage. Otherwise, spawn Scout (with `subagent_type: "vbw:vbw-scout"`) with the pre-computed UAT issue lines (`ID|SEVERITY|DESCRIPTION|FAILED_IN_ROUNDS` from step 2), **ordered by failure_count descending** (step 3), so Scout investigates the relevant code areas for each issue. Pass `<output_path>{round_dir}/P{NN}-R{RR}-RESEARCH.md</output_path>` in the Scout prompt (where `{round_dir}`, `{NN}` = phase number, `{RR}` = round from step 5) so Scout writes the file directly to the round directory. Before composing the Scout task description, evaluate installed skills visible in your system context — read each skill's description and determine if it is relevant to this specific task. If any skills are relevant, the Scout prompt MUST start with `<skill_activation>{For each relevant skill: "Call Skill({skill-name})"}</skill_activation>`. Only include skills whose description matches the task at hand. If no skills are relevant, omit the skill_activation block entirely.
   ...
   - `plan`: If `plan_path` from step 5 is non-empty, the plan was already written in a previous session — do NOT re-plan. Read the existing plan and advance directly to `execute`. Otherwise, spawn Lead to create a plan at `{round_dir}/P{NN}-R{RR}-PLAN.md` (where `{round_dir}`, `{NN}`, `{RR}` are from step 5). "No separate discussion" means skip Discuss assumption-gathering — the UAT report serves as scope. For research context, use `research_path` from step 5: if non-empty, include it in Lead's context (no re-run of Scout needed); if empty, skip research. **Pass the priority-ranked issue list (step 3) to Lead** with recurring-issue annotations. After planning completes, advance:
     ```bash
     bash /tmp/.vbw-plugin-root-link-${CLAUDE_SESSION_ID:-default}/scripts/uat-remediation-state.sh advance "$PHASE_DIR"
     ```
