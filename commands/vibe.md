...
3. **Research persistence (REQ-08, graduated):** If effort != turbo:
   - Determine the next plan number `{MM}` and resolve artifact paths:
     ...
     ```
   - Check for phase-wide research `{phase-dir}/${RESEARCH_NAME}` (preferred). If phase-wide does not exist, check for any per-plan brownfield research: `BROWNFIELD_RESEARCH=$(find "{phase-dir}" -maxdepth 1 -name "{NN}-[0-9][0-9]-RESEARCH.md" -print -quit 2>/dev/null)`. If `$BROWNFIELD_RESEARCH` is non-empty, use that path.
