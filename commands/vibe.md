...
3. **Compile context:** If `config_context_compiler=true`, run:
   ...
   If todo detail was loaded during Input Parsing (ref tag extraction), include it in the Dev task description: "Extended context (from todo detail): {detail.context value}. Related files: {detail.files, comma-separated, or omit if empty}." This provides the executing agent with the rich context that motivated the work item.
